...
This site deploys automatically when changes are pushed to `master`, so avoid accidental deployments.

- Always create a new branch before making project changes.
- Do not push directly to `master`.
- Do not merge back to `master` unless Jiaji explicitly confirms the merge/deploy.
- PRs are optional for this personal site; direct branch-to-master merges are acceptable only after explicit confirmation.

Resolve any conflicts before merge or deploy.
